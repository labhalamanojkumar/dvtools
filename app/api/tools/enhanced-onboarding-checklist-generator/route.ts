...
interface EnhancedChecklistTask {
  id: string;
  title: string;
  description: string;
...
interface EnhancedChecklistTemplate {
  id: string;
  name: string;
  description: string;
  category: string;
  tasks: EnhancedChecklistTask[];
  createdAt: string;
  updatedAt: string;
  version: string;
  tags: string[];
...
interface EnhancedAssignedChecklist {
  id: string;
  templateId: string;
  templateName: string;
  assignedTo: string;
...
const templates = new Map<string, EnhancedChecklistTemplate>();
const assignments = new Map<string, EnhancedAssignedChecklist>();
...
const initializeEnhancedSampleData = () => {
  if (templates.size === 0) {
    const enhancedSampleTemplates: EnhancedChecklistTemplate[] = [
      {
        id: "1",
        name: "Software Engineer Onboarding",
        description: "Complete onboarding checklist for new software engineers with technical setup",
        category: "employee",
    ...
    enhancedSampleTemplates.forEach(template => templates.set(template.id, template));

    // Add enhanced sample assignments
    const template = templates.get("1");
    if (template) {
      const enhancedAssignment: EnhancedAssignedChecklist = {
        id: "a1",
...
const calculateEnhancedProgress = (tasks: EnhancedChecklistTask[]): number => {
  if (tasks.length === 0) return 0;
  const completed = tasks.filter(t => t.completed).length;
  return Math.round((completed / tasks.length) * 100);
};

const calculateEnhancedStatus = (progress: number, dueDate: string): "not-started" | "in-progress" | "completed" | "overdue" => {
...
const calculateAdvancedAnalytics = (): AdvancedAnalytics => {
  ...
  return {
    ...
    bottlenecks: bottlenecks.slice(0, 5).map(b => ({ ...b, avgTime: parseInt(b.avgTime) || 0 })),
  };
};
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    switch (action) {
      case "getTemplates": {
        const { search, category, difficulty } = body;
        let filteredTemplates = Array.from(templates.values()).sort((a, b) => 
      ...
      case "createTemplate": {
        const { name, description, category, difficulty, tags, tasks, estimatedTotalTime } = body;
        
        const template: EnhancedChecklistTemplate = {
          id: Date.now().toString(),
          name,
          description,
          category,
          ...
          version: "1.0",
          tags: tags || [],
          tasks: tasks.map((task: EnhancedChecklistTask, index: number) => ({
            ...task,
            completed: false,
            order: index,
            createdAt: new Date().toISOString().split("T")[0],
      ...
      case "updateTemplate": {
        const { templateId, name, description, category, difficulty, tags, tasks } = body;
        ...
        const newEstimatedTotalTime = tasks.reduce((total: number, task: EnhancedChecklistTask) => {
        ...
        template.tags = tags || [];
        template.tasks = tasks.map((task: EnhancedChecklistTask, index: number) => ({
          ...task,
          completed: false,
          order: index,
          updatedAt: new Date().toISOString().split("T")[0],
        ...
        template.estimatedTotalTime = `${newEstimatedTotalTime} hours`;
      ...
      case "deleteTemplate": {
        ...
        broadcastUpdate('template_deleted', { templateId });
      ...
      case "importTemplates": {
        ...
        importedTemplates.forEach((template: EnhancedChecklistTemplate) => {
          const newTemplate: EnhancedChecklistTemplate = {
            ...template,
        ...
        broadcastUpdate('templates_imported', { count: importedTemplates.length });
      ...
      case "getAssignments": {
        ...
        let filteredAssignments = Array.from(assignments.values()).sort((a, b) => 
      ...
      case "assignChecklist": {
        const { templateId, assigneeName, assigneeEmail, dueDate, notifications: enableNotifications } = body;
        ...
        const assignment: EnhancedAssignedChecklist = {
          id: Date.now().toString(),
          templateId: template.id,
          templateName: template.name,
        ...
        addActivityLog(assignment.id, {
          type: "assignment",
          user: "admin@example.com",
          description: `Assigned "${template.name}" to ${assigneeName}`,
          timestamp: new Date().toISOString(),
        });

        // Create notification if enabled
        if (enableNotifications) {
          createNotification(assigneeEmail, {
            type: "assignment",
            message: `New checklist "${template.name}" assigned to you`,
        ...
        broadcastUpdate('assignment_created', assignment);
      ...
      case "toggleTask": {
        ...
        assignment.progress = calculateEnhancedProgress(assignment.tasks);
        assignment.status = calculateEnhancedStatus(assignment.progress, assignment.dueDate);
        ...
        if (task?.completed) {
          createNotification(assignment.assignedBy, {
            type: "completion",
            message: `${assignment.assignedToName} completed "${task?.title}"`,
        ...
        broadcastUpdate('task_updated', { assignmentId, taskId, task });
      ...
      case "addComment": {
        ...
        createNotification(assignment.assignedTo, {
          type: "comment",
          message: `New comment on "${assignment.templateName}"`,
        ...
        broadcastUpdate('comment_added', { assignmentId, comment });

        return NextResponse.json({ success: true });
      }

      case "bulkOperation": {
        const { action, assignmentIds, taskIds, data } = body as BulkOperation;
        ...
        for (const assignmentId of assignmentIds) {
          ...
          results.push({ assignmentId, status: "success" });
        }

        broadcastUpdate('bulk_operation', { action, assignmentIds });
      ...
      case "reorderTasks": {
        ...
        broadcastUpdate('tasks_reordered', { templateId, taskOrder });
    ...
    console.error("Error in enhanced onboarding-checklist-generator API:", error);
    return NextResponse.json(
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (type === 'stream') {
      ...
      const stream = new ReadableStream({
        start(controller) {
          ...
          sendUpdate({ type: 'connection', data: 'Connected to real-time updates' });

          // Keep connection alive
          const keepAlive = setInterval(() => {
            try {
              // Check if controller is still active before sending keep-alive
    ...
    return NextResponse.json({ message: "Enhanced Onboarding Checklist Generator API" });
  } catch (error) {
