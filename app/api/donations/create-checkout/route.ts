...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      donorName: bodyDonorName,
      donorEmail: bodyDonorEmail,
      email: bodyEmail,
      name: bodyName,
      message,
      tierId,
    ...
    const normalizedGateway = typeof gateway === 'string' 
      ? gateway.toUpperCase() as PaymentGateway
      : gateway
    
    if (!validGateways.includes(normalizedGateway)) {
      return NextResponse.json(
        { 
          error: `Invalid payment gateway: ${gateway}`,
          message: 'Supported gateways: ' + validGateways.join(', '),
          details: 'Please use one of the supported payment gateways'
        },
        { status: 400 }
      )
    }
    ...
    // Use fallbacks for email and name from session or body
    const donorEmail = bodyDonorEmail || bodyEmail || session?.user?.email
    const donorName = bodyDonorName || bodyName || session?.user?.name || 'Anonymous'

    console.log('👤 Resolved donor info:', {
      donorEmail,
      donorName,
      source: bodyDonorEmail ? 'body.donorEmail' : bodyEmail ? 'body.email' : session?.user?.email ? 'session' : 'none'
    })
    ...
    if (!donorEmail) {
      return NextResponse.json(
        { 
          ...
          message: 'Please provide your email address or login to continue',
          details: 'Email is required for payment processing and receipts'
        },
        { status: 400 }
      )
    }
    ...
    if (!gatewayConfig) {
      return NextResponse.json(
        { 
          error: `${normalizedGateway} payment gateway is not configured`,
    ...
    if (!gatewayConfig.isEnabled) {
      return NextResponse.json(
        { 
          error: `${normalizedGateway} payment gateway is currently disabled`,
    ...
    if (!gatewayConfig.secretKey) {
      return NextResponse.json(
        { 
          error: `${normalizedGateway} payment gateway is not properly configured`,
    ...
    if (!gatewayConfig.publicKey && (normalizedGateway === 'RAZORPAY' || normalizedGateway === 'PAYPAL')) {
      return NextResponse.json(
        { 
          error: `${normalizedGateway} payment gateway is not properly configured`,
          message: 'Missing public key/API key. Please add your public key in the admin panel',
          details: 'Public key is required for this gateway but not configured'
        },
        { status: 400 }
      )
    }
    ...
    console.log('🔧 Initializing payment service for gateway:', normalizedGateway)
    
    try {
      PaymentService.initialize(normalizedGateway, {
    ...
    let checkoutSession
    try {
      checkoutSession = await PaymentService.createCheckout(normalizedGateway, checkoutParams)
